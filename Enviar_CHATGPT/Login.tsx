...
export default function Login() {
    console.log('Login component mounted - vPatchSenior');
    const navigate = useNavigate();
    const { authReady, session } = useUser();
    const [email, setEmail] = useState('');
    const [password, setPassword] = useState('');
    const [loading, setLoading] = useState(false);
    const [error, setError] = useState<string | null>(null);

    // ✅ FIX: Declarative redirection based on context state
    // Prevents loops and race conditions by trusting the central source of truth
    useEffect(() => {
        if (authReady && session) {
            console.log('[Login] Session active, redirecting to /');
    ...
    async function handleLogin(e: React.FormEvent) {
        ...
        try {
            ...
            console.log('[Login] Sign-in successful (awaiting context update)');

            // No navigate() here. We wait for onAuthStateChange -> UserContext -> useEffect above.
            ...
            <div className="relative z-10 w-full max-w-md mx-auto">
                <div className="text-center mb-10">
                    ...
                    <h1 className="text-3xl font-black text-slate-900 dark:text-white mb-2 tracking-tight">
                        TEMI <span className="text-primary">Quest</span>
                    </h1>
                    <p className="text-slate-500 dark:text-slate-400">Entre para sincronizar seu progresso</p>
                </div>
                    ...
                    <form onSubmit={handleLogin} className="space-y-6">
                        ...
                        <div className="space-y-2">
                            <div className="flex justify-between items-center ml-1">
                                ...
                                <button
                                    type="button"
                                    onClick={() => alert('Funcionalidade em breve: Verifique seu e-mail para redefinir.')}
